<x-landing.app-layout :title="$blog->title">
                    ...
                    <article class="article-detail" data-aos="fade-up">
                        ...
                        <div class="mb-3">
                            <span class="badge" style="background-color: #349953; font-size: 14px; padding: 8px 16px;">
                                {{-- Gunakan nama_kategori sesuai kolom di BlogCategory --}}
                                <i class="bi bi-folder me-1"></i> {{ $blog->category->nama_kategori }}
                            </span>
                        </div>
                        @endif
                        ...
                        <!-- Main Featured Image — gunakan kolom `image` -->
                        @if($blog->image)
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    <div class="card border-0 shadow-sm hover-card h-100">
                                        {{-- Gunakan `image` sebagai gambar artikel terkait --}}
                                        @if($related->image)
                                        <div style="height: 200px; background: url('{{ asset($related->image) }}') center/cover; border-radius: 8px 8px 0 0;"></div>
                                        @else
                                        <div style="height: 200px; background: url('{{ asset('MediTrust/assets/img/gallery/gallery-1.webp') }}') center/cover; border-radius: 8px 8px 0 0;"></div>
                                        @endif
                                        <div class="card-body">
                                            <span class="badge mb-2" style="background-color: rgba(52, 153, 83, 0.1); color: #349953;">
                                                <i class="bi bi-folder me-1"></i>{{ $related->category?->nama_kategori ?? 'Umum' }}
                                            </span>
    ...
    <!-- Additional CSS -->
    <style>
        .article-detail { background: white; border-radius: 15px; padding: 40px; box-shadow: 0 5px 20px rgba(0,0,0,0.05); }
        .article-title { font-size: 2.2rem; font-weight: 700; line-height: 1.3; color: #2c3e50; }
        .article-meta { font-size: 14px; }
        .featured-image img { transition: transform 0.3s ease; border-radius: 12px; }
        .featured-image:hover img { transform: scale(1.02); }
        .article-body h2, .article-body h3, .article-body h4 { color: #349953; margin-top: 2rem; margin-bottom: 1rem; }
        ...
        .hover-card { transition: all 0.3s ease; height: 100%; }
        .hover-card:hover { transform: translateY(-5px); box-shadow: 0 10px 30px rgba(52,153,83,0.2) !important; }
        ...
        @media (max-width: 768px) {
            .article-meta { flex-direction: column; align-items: flex-start !important; }
        }
    </style>
