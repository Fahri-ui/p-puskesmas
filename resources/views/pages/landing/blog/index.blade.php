        ...
        <div class="container">
            <div class="breadcrumbs">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('welcome') }}"><i class="bi bi-house"></i>
                                Beranda</a></li>
            ...
            <div class="title-wrapper text-center mt-4">
                <h1>Berita</h1>
                <p class="mt-2">Temukan update terkini seputar layanan, kegiatan, dan informasi kesehatan dari
                    Puskesmas Binong.</p>
            </div>
        </div>
    </div><!-- End Page Title -->
                    ...
                    @if ($featuredNews)
                        <div class="featured-news mb-5" data-aos="fade-up">
                            <div class="card border-0 shadow-lg overflow-hidden">
                                <div class="row g-0">
                                    <div class="col-md-6">
                                        <div class="featured-image position-relative overflow-hidden"
                                            style="height: 100%; min-height: 350px; position: relative;">
                                            @if ($featuredNews->image)
                                                <img src="{{ asset($featuredNews->image) }}"
                                                    alt="{{ $featuredNews->title }}" class="w-100 h-100"
                                                    style="object-fit: cover;">
                                            @else
                                                <div class="image-placeholder d-flex align-items-center justify-content-center w-100 h-100 bg-light text-center px-3"
                                                    style="border-radius: 0;">
                                                    <span class="text-muted">{{ $featuredNews->title }}</span>
                                                </div>
                                            @endif

                                            <div class="featured-badge"
                                                style="position: absolute; top: 20px; left: 20px; background-color: #349953; color: white; padding: 8px 20px; border-radius: 20px; font-size: 13px; font-weight: 600;">
                                        ...
                                        <div class="card-body p-4 d-flex flex-column h-100">
                                            <div class="mb-2">
                                                <span class="badge mb-2"
                                                    style="background-color: rgba(52, 153, 83, 0.1); color: #349953; font-weight: 500;">
                                                    <i
                                                        class="bi bi-folder me-1"></i>{{ $featuredNews->category?->nama_kategori ?? 'Umum' }}
                                                </span>
                                            </div>
                                            <h3 class="card-title mb-3" style="color: #2c3e50; line-height: 1.4;">
                                                <a href="{{ route('blog.show', $featuredNews->slug) }}"
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <small class="text-muted">
                                                        <i class="bi bi-calendar-event me-1"
                                                            style="color: #349953;"></i>
                                                        {{ $featuredNews->published_at->format('d F Y') }}
                                                    </small>
                                                    <a href="{{ route('blog.show', $featuredNews->slug) }}"
                                                        class="btn btn-sm"
                                                        style="background-color: #349953; color: white;">
                                    ...
                                    <div class="row g-0">
                                        <div class="col-md-4">
                                            ...
                                            <div class="news-thumbnail"
                                                style="height: 100%; min-height: 200px; background: url('{{ $item->image ? asset($item->image) : asset('MediTrust/assets/img/gallery/gallery-1.webp') }}') center/cover; border-radius: 8px 0 0 8px; position: relative;">
                                                <div class="category-badge"
                                                    style="position: absolute; top: 15px; left: 15px; background-color: #349953; color: white; padding: 5px 12px; border-radius: 15px; font-size: 11px; font-weight: 600;">
                            ...
                            <div class="card border-0 shadow-sm">
                                <div class="card-body p-4">
                                    ...
                                    <ul class="list-unstyled mb-0">
                                        <li class="mb-2">
                                            <a href="{{ route('blog') }}"
                                                class="d-flex justify-content-between align-items-center text-decoration-none {{ !$categoryId ? 'text-success fw-bold' : 'text-dark' }} hover-link">
                                                <span>
                                                    <i class="bi bi-chevron-right me-2"
                                                        style="color: {{ !$categoryId ? '#349953' : '#6c757d' }};"></i>Semua
                                                    Kategori
                                                </span>
                                            </a>
                                        </li>
                                        @foreach ($categories as $category)
                                            <li class="mb-2">
                                                <a href="{{ route('blog', ['category' => $category->id]) }}"
                                                    class="d-flex justify-content-between align-items-center text-decoration-none {{ $categoryId == $category->id ? 'text-success fw-bold' : 'text-dark' }} hover-link">
                                                    <span>
                                                        <i class="bi bi-chevron-right me-2"
                                                            style="color: {{ $categoryId == $category->id ? '#349953' : '#6c757d' }};"></i>{{ $category->nama_kategori }}
                                                    </span>
                                                </a>
                                            </li>
                                        @endforeach
                                ...
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body p-4">
                                        ...
                                        @foreach ($popularNews as $popular)
                                            <div
                                                class="popular-item mb-3 pb-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                                <div class="d-flex">
                                                    <div class="popular-thumbnail me-3"
                                                        style="width: 80px; height: 80px; border-radius: 8px; flex-shrink: 0; overflow: hidden; background-color: #f8f9fa;">
                                                        @if ($popular->image)
                                                            <img src="{{ asset($popular->image) }}"
                                                                alt="{{ $popular->title }}" class="w-100 h-100"
                                                                style="object-fit: cover;">
                                                        @else
                                                            <div
                                                                class="image-placeholder d-flex align-items-center justify-content-center w-100 h-100 text-center px-2">
                                                                <span class="text-muted"
                                                                    style="font-size: 12px;">{{ Str::limit($popular->title, 40) }}</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                    <div>
